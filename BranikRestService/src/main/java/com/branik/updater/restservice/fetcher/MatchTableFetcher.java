...
@Service
public class MatchTableFetcher implements TableFetcher {
    ...
    @Autowired
    public MatchTableFetcher(TableService matchDetailsTableService, Config config) {
        this.tableService = matchDetailsTableService;
        this.config = config;
        this.tableHtmlElement = config.getString(ConfigKeys.MATCH_TABLE_HTML_ELEMENT);
        this.urlString = config.getString(ConfigKeys.MATCH_TABLE_URL);
    }
