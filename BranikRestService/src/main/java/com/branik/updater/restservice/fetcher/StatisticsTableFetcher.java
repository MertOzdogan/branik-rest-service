...
@Service
public class StatisticsTableFetcher implements TableFetcher {
    ...
    @Autowired
    public StatisticsTableFetcher(Config config, TableService genericTableService) {
        this.tableService = genericTableService;
        this.config = config;
        this.tableHtmlElement = config.getString(ConfigKeys.STATISTICS_TABLE_HTML_ELEMENT);
        this.urlString = config.getString(ConfigKeys.STATISTICS_TABLE_URL);
    }
